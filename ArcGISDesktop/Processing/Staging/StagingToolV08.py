...
# Lookup Table Path
lookup = r"C:\BaseData\lookup.gdb\lookup"
...
if wirelessInput <>"":
    ...
    # Create list of ProvAlias found in Provider data
    provListRows = arcpy.UpdateCursor(wireless, "", "", "PROVALIAS; PROVNAME; DBANAME; FRN; TYPICUP; TYPICDOWN", "")
    #provListRow = provListRows.next()
    provaliasList = []
    ...
    for provListRow in provListRows:
        ...
        if provListRow.TYPICDOWN not in provaliasList:
            provaliasList.append(provListRow.TYPICDOWN)
    rows = arcpy.UpdateCursor(wireless, "", "", "PROVALIAS; PROVNAME; DBANAME; FRN; TYPICUP; TYPICDOWN", "")
    row = rows.next()
    while row:
...
#    arcpy.AddMessage("Unable to process Address Points")
    ...
    arcpy.AddMessage("WHEN field added")
    provListRows = arcpy.UpdateCursor(address, "", "", "PROVALIAS; PROVNAME; DBANAME; FRN; TYPICUP; TYPICDOWN", "")
    arcpy.AddMessage("Updated Cursor")
    provListRow = provListRows.next()
    provaliasList = []

    #for the current row in "address", print values
    for provListRow in provListRows:
#        print "provalias: %s, provname: %s, dbaname: %s, frn: %s, typicup: %s, typicdown: %s" % \
#            (provListRow.PROVALIAS, provListRow.PROVNAME, provListRow.DBANAME, provListRow.FRN,provListRow.TYPICUP, provListRow.TYPICDOWN)
        if provListRow.PROVALIAS not in provaliasList:
            provaliasList.append(provListRow.PROVALIAS)
            arcpy.AddMessage(provaliasList, " added to ProvAliasList")
            print provListRow.PROVALIAS," added to provaliasList"
        if provListRow.TYPICUP not in provaliasList:
            provaliasList.append(provListRow.TYPICUP)
            arcpy.AddMessage(provListRow.TYPICUP, " added to provaliaslist")
        if provListRow.TYPICDOWN not in provaliasList:
            provaliasList.append(provListRow.TYPICDOWN)
    rows = arcpy.UpdateCursor(address, "", "", "PROVALIAS; PROVNAME; DBANAME; FRN; TYPICUP; TYPICDOWN", "")
    row = rows.next()
    while row:
